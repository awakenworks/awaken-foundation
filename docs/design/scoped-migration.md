...
The closed set, ported from IAM. `{prefix}` is the per-database table prefix; the
...
A bundle's **scope** is the segment of its id before the first `.` (`iam` in
`iam.authz`). All tables share the per-DB `{prefix}`, so ownership cannot be read
...
SQLite and an advisory lock for Postgres, modelled on IAM's `MigrationExecutor`:
...
The convergence is "done" when these hold, mirroring IAM's suite:
